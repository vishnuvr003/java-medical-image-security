...
public class Histogram extends Thread{
  ...
  {
      try {
          File final_image = new File("C:\\Users\\Administrator\\Desktop\\Sini\\download (1).jpg");;
          BufferedImage img = ImageIO.read(final_image);
          int src[]=Utill.convertToPixels(img);
          System.out.print(new Histogram().mini(src, img.getWidth(), img.getHeight()));
   ...
   */
 public double  mini (int [] src, int width, int height){
